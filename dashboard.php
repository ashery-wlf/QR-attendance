...
if ($user_role === 'super_admin') {
    ...
    $orgCountResult = $conn->query("SELECT COUNT(*) AS total FROM organizations WHERE is_active = 1");
    $upcomingCount = $orgCountResult ? (int) $orgCountResult->fetch_assoc()['total'] : 0;
    
    $usersCountResult = $conn->query("SELECT COUNT(*) AS total FROM users WHERE role <> 'super_admin'");
    $endedCount = $usersCountResult ? (int) $usersCountResult->fetch_assoc()['total'] : 0;
    ...
    if ($user_role === 'attendee') {
        // Attendee: Show own joined events
        ...
        $pendingResult = $conn->query("
            ...
            AND a.id IS NULL
            AND e.deleted = FALSE
            AND e.organization_id = $user_org_id
            AND CONCAT(e.date, ' ', COALESCE(e.end_time, e.time), ':00') >= NOW()
        ");
        $pendingCount = $pendingResult ? (int) $pendingResult->fetch_assoc()['total'] : 0;
    } else {
        // Org Admin / Event Organizer: Show org-specific stats
        if ($user_role === 'organization_admin') {
            $eventsQuery = "SELECT COUNT(*) AS total FROM events WHERE organization_id = $user_org_id AND deleted = FALSE";
        } else {
            // Event organizer
            $eventsQuery = "SELECT COUNT(*) AS total FROM events WHERE (created_by = $user_id OR organization_id = $user_org_id) AND deleted = FALSE";
        }
        
        $orgEventsResult = $conn->query($eventsQuery);
        $joinedCount = $orgEventsResult ? (int) $orgEventsResult->fetch_assoc()['total'] : 0;
        
        $attendedResult = $conn->query("
            SELECT COUNT(*) AS total FROM attendance 
            WHERE event_id IN (SELECT id FROM events WHERE organization_id = $user_org_id AND deleted = FALSE)
        ");
        $attendedCount = $attendedResult ? (int) $attendedResult->fetch_assoc()['total'] : 0;
...
// Build events query based on role
$whereConditions = ["e.deleted = FALSE"];

if ($user_role === 'super_admin') {
    // Super admin: Show NO events, only org stats
    $events = null;
} elseif ($user_role === 'attendee') {
    // Attendee: all events from their organization. Joined filters are applied below.
    $whereConditions[] = "e.organization_id = $user_org_id";
} elseif ($user_role === 'organization_admin') {
    // Org Admin: All events from their org
    $whereConditions[] = "e.organization_id = $user_org_id";
} else {
    // Event Organizer: Events they created or their org
    $whereConditions[] = "(e.created_by = $user_id OR e.organization_id = $user_org_id)";
}

if ($search !== "") {
    $whereConditions[] = "(e.name LIKE '%$safeSearch%' OR e.access_code LIKE '%$safeSearch%' OR e.venue_name LIKE '%$safeSearch%' OR e.venue_location LIKE '%$safeSearch%')";
}

if ($filter === "upcoming" && $user_role !== 'super_admin') {
    $whereConditions[] = "CONCAT(e.date, ' ', e.time, ':00') >= NOW()";
}

if ($user_role === 'attendee' && $filter === 'joined') {
    $whereConditions[] = "p.id IS NOT NULL";
}

if ($user_role === 'attendee' && $filter === 'not_joined') {
...
renderAppShellStart($conn, [
    ...
    "page_subtitle" => $user_role === 'super_admin' 
        ? "System overview and organization management."
        : ($user_role === 'organization_admin' 
            ? "Manage your organization, venues, and events."
            : ($user_role === 'event_organizer'
                ? "Create and manage your events and attendance."
                : "Track your joined events, attendance, and available events.")),
...
<section class="mobile-hello">
    ...
    <p><?php 
        if ($user_role === 'super_admin') {
            echo 'System overview and management.';
        } elseif ($user_role === 'organization_admin') {
            echo 'Manage your organization and events.';
        } elseif ($user_role === 'event_organizer') {
            echo 'Manage your events and attendance.';
        } else {
    ...
    <section class="stats-grid">
        <article class="stat-card">
            ...
            <div>
                ...
                <span class="flat-note"><a href="organizations.php" style="color:#5448f6;font-weight:700;">Manage →</a></span>
            </div>
        </article>

        <article class="stat-card">
            <div class="stat-icon mint"><?php echo appIcon("profile"); ?></div>
            <div>
                <h3>Total Users</h3>
                <strong><?php echo $endedCount; ?></strong>
                <span class="flat-note">All non-admin users</span>
            </div>
        </article>

        <article class="stat-card">
            <div class="stat-icon amber"><?php echo appIcon("events"); ?></div>
            <div>
                <h3>Total Events</h3>
                <strong><?php echo $liveCount; ?></strong>
                <span class="flat-note"><a href="events.php" style="color:#f59e0b;font-weight:700;">View all →</a></span>
            </div>
        </article>
    </section>
<?php elseif ($user_role === 'organization_admin' || $user_role === 'event_organizer'): ?>
    <!-- Org Admin / Event Organizer Dashboard -->
    <section class="stats-grid">
        <article class="stat-card">
            ...
            <div>
                ...
                <span class="flat-note"><?php echo $user_role === 'organization_admin' ? 'Your organization' : 'Created or managed'; ?></span>
            </div>
        </article>
        ...
        <article class="stat-card">
            ...
            <div>
                ...
                <span class="flat-note">Across all events</span>
            </div>
        </article>
        ...
        <article class="stat-card">
            ...
            <div>
                ...
                <strong><?php echo $upcomingCount; ?></strong>
                <span class="flat-note">Scheduled</span>
            </div>
        </article>
    </section>
<?php else: ?>
...
<section class="board">
    <div class="board-head">
        <div>
            <h2><?php echo $user_role === 'attendee' ? 'Available Events' : 'Your Events'; ?></h2>
            <p><?php echo $user_role === 'attendee' ? 'Discover and join events using a secure code or self registration.' : 'Manage and track your events.'; ?></p>
        </div>
...
<?php endif; // end of $user_role !== 'super_admin' ?>
