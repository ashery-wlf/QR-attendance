...
if (isset($_POST['delete_event']) && is_numeric($_POST['delete_event'])) {
    ...
    $eventCheck = $conn->query("SELECT created_by, organization_id FROM events WHERE id = $eventId LIMIT 1");
    if ($eventCheck && $eventCheck->num_rows > 0) {
        $event = $eventCheck->fetch_assoc();
        // Super admin can delete any, org admin can delete org events, organizer can delete own
        $canDelete = false;
        if ($user_role === 'super_admin') {
            $canDelete = true;
        } elseif ($user_role === 'organization_admin' && (int) $event['organization_id'] === $user_org_id) {
            $canDelete = true;
        } elseif ((int) $event['created_by'] === $user_id) {
            $canDelete = true;
        }
...
if ($user_role === 'super_admin') {
    ...
    // Can see only events from their organization
    $where .= " AND e.organization_id = $user_org_id";
} elseif ($user_role === 'event_organizer') {
    // Can see events they created or events from their organization
    $where .= " AND (e.created_by = $user_id OR e.organization_id = $user_org_id)";
} else {
    // Attendee: can see events from their organization
    $where .= " AND e.organization_id = $user_org_id";
}

if ($search !== "") {
    $where .= " AND (e.name LIKE '%$safeSearch%' OR e.venue_name LIKE '%$safeSearch%' OR e.venue_location LIKE '%$safeSearch%')";
}
